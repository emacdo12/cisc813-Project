...
PRINTER_TRIG = 2
GOOGLE_HOME_TRIG = 2
SMART_SPEAKER_TRIG = 2



# This file is for generating problem files for the scenario where 
# we are scaling the number of devices 
...
num_devices = num_doorbells + num_security_camera + num_printers  + num_smart_speaker + num_light_sw # + num_google_homes
all_devices = np.repeat("security_cam00",num_devices)
...
# Choose speaker as initially compromised machine
for i, device in enumerate(all_devices):
    if("speaker" in device):
        compromised_idx = i
        break
...
for i, device in enumerate(all_devices):
    if("cam" in device):
        vuln_idx[i] = True
    if(("speaker" in device) and (not("speaker0" in device))):
        vuln_idx[i] = True

print(vuln_idx)
vuln_devices = all_devices[vuln_idx == 1]
...
file_name = os.path.join("Temporal\problem_files","promblem.home.pddl")
...
################## domain & objects ######################################
    ...
    for i in range(num_smart_speaker):
        file.write("\n\t\t; speaker " + str(i))
        file.write("\n\t\t(trigger_at_time speaker" + str(i) + " am8)")
        file.write("\n\t\t(trigger_at_time speaker" + str(i) + " am7)")
    ...
    for i in range(num_light_sw):
        ...
        file.write("\n\t\t(=(max_trigger_count light_sw" + str(i) + ") " + str(LIGHT_TRIGGER_COUNT) + ")")
    ...
    for i in range(num_vuln):
        if(("doorbell" in vuln_devices[i]) | ("light_sw" in vuln_devices[i])):
            file.write("\n\t\t(does_encrypt " + vuln_devices[i] + ")" )
            file.write("\n\t\t(open_TCP_23_port " + vuln_devices[i]  + ")" )
        if(("security_cam" in vuln_devices[i]) | ("speaker" in vuln_devices[i])):
            file.write("\n\t\t(has_pass " + vuln_devices[i] + " weak_password)")
        if(("printer" in vuln_devices[i]) |( "google_home" in vuln_devices[i])):
            file.write("\n\t\t(no_auth_firmwear " + vuln_devices[i] + ")")

    file.write("\n\n\t\t;initial compromised device")
    file.write("\n\t\t(is_compromised " + all_devices[compromised_idx] + ")")
